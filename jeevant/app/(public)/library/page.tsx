...
export default async function LibraryPage() {
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top,rgba(168,85,247,0.18),transparent_35%),linear-gradient(180deg,#140f18,#06060a)] p-8 shadow-2xl sm:p-10">
        <div className="max-w-3xl space-y-4">
          <p className="text-xs font-semibold uppercase tracking-[0.35em] text-violet-300">Library</p>
          <h1 className="text-4xl font-black tracking-tight text-white sm:text-6xl">The books, references, and learning trail.</h1>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">Books, articles, and longer-form material belong here.</p>
        </div>
        <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6">
          <BookMarked className="h-6 w-6 text-sky-300" />
          <h2 className="mt-4 text-xl font-bold text-white">Tracked learning</h2>
          <p className="mt-2 text-sm leading-6 text-slate-400">Courses and resources can be tracked with backend data.</p>
        </div>
        <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.03] p-6">
          <BookOpenText className="h-6 w-6 text-amber-300" />
          <h2 className="mt-4 text-xl font-bold text-white">Useful notes</h2>
          <p className="mt-2 text-sm leading-6 text-slate-400">Takeaways from what you read can stay with the source.</p>
        </div>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 sm:p-8">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <h2 className="text-2xl font-bold text-white">Learning catalog</h2>
            <span className="text-xs uppercase tracking-[0.3em] text-slate-500">Backend</span>
          </div>
          <div className="mt-6 space-y-4">
            {learning.length === 0 ? (
              <div className="text-sm text-slate-500">Add books, courses, or papers from the admin area.</div>
            ) : (
              learning.map((item: any) => (
                <div key={item._id} className="rounded-2xl border border-white/10 bg-black/30 p-5">
        ...
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 sm:p-8">
          ...
          <div className="mt-6 space-y-4">
            {notes.length === 0 ? (
              <div className="text-sm text-slate-500">Use notes to store summaries, insights, or quotes from what you read.</div>
            ) : (
              notes.slice(0, 6).map((note: any) => (
                <div key={note._id} className="rounded-2xl border border-white/10 bg-black/30 p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-400">{note.content || "Convert this into a paper summary or reading note."}</p>
                </div>
              ))
            )}
          </div>
